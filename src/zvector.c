...
static void _vect_destroy(vector v, uint32_t flags) {
  ...
  if ((v->size > 0) && (flags & 1)) {
    ...
    zvect_index i =
        v->size; // if v->size is 200, then the first i below will be 199
    while (i--) {
      if (v->data[i] != NULL) {
        if ((v->flags & ZV_SEC_WIPE))
...
#if (ZVECT_THREAD_SAFE == 1)
void vect_lock_enable(void) { lock_enabled = true; }

void vect_lock_disable(void) { lock_enabled = false; }

inline void vect_lock(vector v) { check_mutex_lock(v, 3); }

inline void vect_unlock(vector v) { check_mutex_unlock(v, 3); }
#endif
...
void *vect_get(vector v) { return _vect_get_at(v, v->size - 1); }

void *vect_get_at(vector v, const zvect_index i) { return _vect_get_at(v, i); }

void *vect_get_front(vector v) { return _vect_get_at(v, 0); }
...
void vect_put_front(vector v, const void *value) { _vect_put_at(v, value, 0); }
...
static inline void *_vect_remove_at(vector v, const zvect_index i) {
  ...
  if ((i < (v->size - 1)) && (v->size > 0)) {
    ...
    if (i > 0)
      vect_memcpy(new_data, v->data, sizeof(void *) * i);
    vect_memcpy(new_data + i, v->data + (i + 1),
                sizeof(void *) * (v->size - i));
#else
    // We can't use the vect_memcpy when not in full reentrant code
    // because it's not safe to use it on the same src and dst.
    vect_memmove(v->data + i, v->data + (i + 1),
                 sizeof(void *) * (v->size - i));
#endif
  }
...
inline void *vect_pop(vector v) { return _vect_remove_at(v, v->size - 1); }

void *vect_remove(vector v) { return _vect_remove_at(v, v->size - 1); }
...
static inline void _vect_delete_at(vector v, const zvect_index start,
  ...
  if (tot_items < (v->size - 1)) {
    ...
    vect_memmove(v->data + start, v->data + (tot_items + 1),
                 sizeof(void *) * (v->size - start));
  }
...
void vect_delete(vector v) { _vect_delete_at(v, v->size - 1, 0); }

void vect_delete_at(vector v, zvect_index i) { _vect_delete_at(v, i, 0); }
...
void vect_delete_front(vector v) { _vect_delete_at(v, 0, 0); }
